...
class AppStore extends Component {
  ...
  onClickCategory = tabId => this.setState({tabId})
          ...
          <ul className="tabs-container">
            {tabsList.map(eachItem => (
              <TabItem
                ...
                isActive={tabId === eachItem.tabId}
              />
            ))}
          </ul>
          <ul className="apps-list-container">
            {filterAppList.map(eachItem => (
              <AppItem key={eachItem.appId} appDetail={eachItem} />
            ))}
          </ul>
        </div>
      </div>
...
// solution by using methods by ccbp
//   state = {
//     searchInput: '',
